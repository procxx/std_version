...
#define GENERATE_COMPARISON_TEST_CASES(classname, val_less, val_eq, val_greater)                                       \
  TEST_CASE("Comparison tests for " #classname) {                                                                      \
    ...
    SUBCASE("equal") { REQUIRE_EQ(ver, ver_eq); }                                                                      \
                                                                                                                       \
    SUBCASE("not equal") { REQUIRE_NE(ver, ver_greater); }                                                             \
                                                                                                                       \
    SUBCASE("greater") { REQUIRE(ver_greater > ver); }                                                                 \
                                                                                                                       \
    SUBCASE("less") { REQUIRE(ver_less < ver); }                                                                       \
                                                                                                                       \
...
static_assert(
    std::is_same_v<::STD_VERSION_NS::version_traits<::STD_VERSION_NS::simple_version>::value_type, std::size_t>);
